...
public interface LongTermMemoryMapper {
    ...
    @Insert("INSERT INTO long_term_memory " +
            "(content, memory_type, scope, project_id, document_id, session_id, " +
            "embedding, embedding_model, importance, metadata, created_at, updated_at) " +
            "VALUES (#{content}, #{memoryType}, #{scope}, " +
            "CAST(#{projectId} AS uuid), CAST(#{documentId} AS uuid), CAST(#{sessionId} AS uuid), " +
    ...
    @Select("SELECT id, content, memory_type AS memoryType, scope, " +
            "project_id AS projectId, document_id AS documentId, session_id AS sessionId, " +
    ...
    @Select("<script>" +
            ...
            "  <if test='documentId != null'>OR (scope = 'DOCUMENT' AND document_id = CAST(#{documentId} AS uuid))</if> " +
            ") " +
    ...
    @Select("<script>" +
            ...
            "<if test='projectId != null'>AND project_id = CAST(#{projectId} AS uuid)</if>" +
            "<if test='documentId != null'>AND document_id = CAST(#{documentId} AS uuid)</if>" +
    ...
    @Update("<script>" +
            ...
            "<if test='documentId != null'>document_id = CAST(#{documentId} AS uuid),</if>" +
